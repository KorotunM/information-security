"""Экран обобщённого мультипликативного рюкзака."""
...
class GeneralizedMultiplicativeScreen(ScrollScreen):
    """Экран GMKP/GMKR."""

    def __init__(
        self,
        service: GeneralizedMultiplicativeKnapsackService,
        on_back: Callable[[], None],
        logger: logging.Logger,
        parent: QWidget | None = None,
    ) -> None:
        super().__init__(
            title="Обобщённый мультипликативный рюкзак (GMKP / GMKR)",
            subtitle="",
            logger=logger,
            parent=parent,
        )
    ...
    def show_steps(self) -> None:
        self.show_steps_dialog("Шаги GMKP")
    ...
    def _require_key_pair(self) -> MultiplicativeKnapsackKeyPair:
        if self.current_key_pair is None:
            raise InputValidationError("Сначала сгенерируйте ключи GMKP.")
